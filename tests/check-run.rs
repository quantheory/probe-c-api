...
fn check_run_pass() {
    ...
    assert!(probe.check_run("int main() { return 0; }".as_bytes())
...
fn check_run_fail() {
    let probe = <Probe>::default();
    assert!(!probe.check_run("int main() { return 1; }".as_bytes())
                  .unwrap().run_output.unwrap().status.success());
}
...
fn check_run_compile_fail() {
    ...
    let compile_run_output =
        probe.check_run("I don't think this is a C program.".as_bytes())
             .unwrap();
